...
def main():
    ...
    if args.predict is None:
        word2id, id2word, category2id, id2category, file2lable, max_len = process_data(args.file_path)
        #print(id2category)
        # if args.max_len <= 0:
        #     args.max_len = max_len
        train_iter, test_iter = load_data(file2lable, word2id, args)
        if not os.path.isdir(args.vocab):
            os.makedirs(args.vocab)
        with open(args.vocab + '/' + 'word2id.pkl', 'wb') as f:
            print('Saving word2id...')
            pickle.dump(word2id, f)
        with open(args.vocab + '/' + 'id2category.pkl', 'wb') as f:
            print('Saving id2category...')
            pickle.dump(id2category, f)
    else:
        with open(args.vocab + '/' + 'word2id.pkl', 'rb') as f:
            print('Loading word2id...')
            word2id = pickle.load(f)
        with open(args.vocab + '/' + 'id2category.pkl', 'rb') as f:
            print('Loading id2category...')
            id2category = pickle.load(f)

    args.embed_num = len(word2id)
    args.class_num = len(id2category)
    ...
    if args.predict is not None:
        print('\ntext:\n%s\ncategory:%s'
              % (args.predict, predict(args.predict, cnn, word2id, id2category, args.cuda)))
    else:
        print()
        try:
